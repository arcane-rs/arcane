...
/// Trait for reflection over [`Name`]s and [`Version`]s.
...
pub mod codegen {
    ...
    ///
    /// **Note:**: This trait is generated by `#[derive(event::Versioned)]` and
    ///            it is not part of the public API. Do not use it directly,
    ///            instead use [`VersionedNames`].
    ///
    /// [`Event`]: super::Event
    pub trait Events {
    ...
    ///           [`VersionedNames::versioned_names`].
    ///
    /// [`Event`]: super::Event
    #[derive(Clone, Debug)]
    ...
    impl VersionedNamesIterator {
        /// Creates a new [`VersionedNamesIterator`] from the given
        /// [`Events::EVENTS`].
    ...
    impl Iterator for VersionedNamesIterator {
        ...
        fn next(&mut self) -> Option<Self::Item> {
            self.0.next().map(|(_, name, ver)| {
                ...
                //         constructed only inside of this module and only
                //         from `Events::EVENTS` which is guaranteed to
                //         contain only valid `Version`s.
